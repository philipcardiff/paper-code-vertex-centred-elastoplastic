...
#include "vertexCentredLinGeomPressureDisplacementSolidMMS.H"
...
defineTypeNameAndDebug(vertexCentredLinGeomPressureDisplacementSolidMMS, 0);
...
void vertexCentredLinGeomPressureDisplacementSolidMMS::setFixedDofs
(
...
void vertexCentredLinGeomPressureDisplacementSolidMMS::enforceTractionBoundaries
(
    const pointVectorField& pointD,
    ...
    forAll(pointD.boundaryField(), patchI)
    {
        ...
        )
        {
            ...
            if (gMin(nPointsPerDualFace) < 1)
            {
                FatalErrorIn
                (
                    "void vertexCentredLinGeomPressureDisplacementSolidMMS::"
...
bool vertexCentredLinGeomPressureDisplacementSolidMMS::converged
(
    const label iCorr,
...
tmp<vectorField>
vertexCentredLinGeomPressureDisplacementSolidMMS::residualD
(
    const pointVectorField& pointD,
    const pointScalarField& pointP
) const
...
tmp<scalarField>
vertexCentredLinGeomPressureDisplacementSolidMMS::residualP
(
    const pointVectorField& pointD,
    const pointScalarField& pointP
) const
...
void vertexCentredLinGeomPressureDisplacementSolidMMS::finiteDiffMatrix
(
...
vertexCentredLinGeomPressureDisplacementSolidMMS::vertexCentredLinGeomPressureDisplacementSolidMMS
(
        Time& runTime,
        const word& region
)
...
bool vertexCentredLinGeomPressureDisplacementSolidMMS::evolve()
{
    ...
    do
    {
        ...
        if (Switch(solidModelDict().lookup("finiteDifferenceJacobian")))
        {
            ...
            vertexCentredLinGeomPressureDisplacementSolidMMS::finiteDiffMatrix
            (
        ...
        if (debug)
        {
            Info<< "bool vertexCentredLinGeomPressureDisplacementSolidMMS::evolve(): "
...
#else
            FatalErrorIn("vertexCentredLinGeomPressureDisplacementSolidMMS::evolve()")
...
void vertexCentredLinGeomPressureDisplacementSolidMMS::setTraction
(
...
)
{
    ...
    else
    {
        FatalErrorIn
        (
            "void Foam::vertexCentredLinGeomPressureDisplacementSolidMMS::setTraction\n"
            "(\n"
...
void vertexCentredLinGeomPressureDisplacementSolidMMS::writeFields
(
    const Time& runTime
)
{
    ...
    const scalar lambda = 1.15e11;
    const scalar mu = 7.69e10;
